...
#SBATCH --gres=gpu:8
#SBATCH --job-name=imagenet-resnet50
#SBATCH --cpus-per-task=12
#SBATCH --output=/tmp/imagenet-resnet50/out.%j.log
#SBATCH --error=/tmp/imagenet-resnet50/err.%j.log

# ResNet-50 training with unstructured sparsity
...
TRAIN_DIR=/tmp/imagenet-resnet50/${SLURM_JOB_ID}
mkdir -p ${TRAIN_DIR}
...
python -m experiments.train \
  ...
  data.batch_size_per_device=512 \
  ...
  sparsifier.sinkhorn_beta_target=10 \
  bias_weight_decay=0 \
  norm_weight_decay=0
