...
#SBATCH --gres=gpu:8
#SBATCH --job-name=imagenet-vitb16-16x16
#SBATCH --cpus-per-task=12
#SBATCH --output=/tmp/imagenet-vitb16-16x16/out.%j.log
#SBATCH --error=/tmp/imagenet-vitb16-16x16/err.%j.log

# ViT-B/16 training with 16x16 block structured sparsity (excludes input conv layer and output head layer)
...
TRAIN_DIR=/tmp/imagenet-vitb16-16x16/${SLURM_JOB_ID}
mkdir -p ${TRAIN_DIR}
...
python -m experiments.train \
  ...
  sparsifier.module_prefixes=[encoder] \
  sparsifier.block_dims=[16,16] \
